...
def train(theta, X, y, model):
     ...
     for i in range(EPOCHS):
        p_y=predict(X,theta)#p_y=array of y(0,1)
        #print(p_y)
        #cost=costFunc(m,y,p_y)
        #J.append(cost)
        #
        cal_gra=calcGradients(X,y,p_y,m)
...
##    return n
    ...
    y_binary=final_y(y_predicted)
    cost=0-((y_binary*np.log(p))+((1-y_binary)*np.log(1-p)))
    cost_1=np.sum(cost)
    return cost_1/m
...
##    th=0
##    y_p=(y>0)#true if y>0
##    y_p=y_p.astype(int)#converting bool to int
    return y
